...
def load_offline_ontology() -> ontopy.ontology.Ontology:
    ...
    magmo_ttl = f"file://{ontology_dir / 'magmo-inferred.ttl'!s}"
    logger.debug("loading magmo from '%s'", magmo_ttl)
    # no internet access is required to resolve the inferred ontology
    magmo = w.get_ontology(magmo_ttl).load()
    return magmo
...
def search_labels(text: str, auto_wildcard: bool = True) -> list[str]:
    ...
    return sorted(
        str(thing.get_preferred_label()) for thing in possible_things if hasattr(thing, "get_preferred_label")
    )
