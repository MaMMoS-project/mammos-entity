...
function requestLazyNode(model, root, lazyId, { force = false, loadAll = false } = {}) {
  ...
  node.dataset.lazyRequestId = String(requestId);
...
function invalidateLazyRequest(node) {
  delete node.dataset.lazyRequestId;
...
function resetCollectionNode(node) {
  ...
  invalidateLazyRequest(node);
}

function responseMatchesRequest(node, message) {
  return node.dataset.lazyRequestId === String(message.request_id);
}
...
export function render({ model, el }) {
  ...
  const handleToggle = (event) => {
    ...
    if (!target.open) {
      if (target.dataset.lazyPatch === "append-children") {
        ...
        model.send({
          kind: "release-lazy",
          lazy_id: target.dataset.lazyId,
        });
      } else {
        invalidateLazyRequest(target);
      }
      return;
    }
    if (root) {
  ...
  const handleMessage = (message) => {
    ...
    if (message.kind === "lazy-rendered") {
      ...
      if (!node || !responseMatchesRequest(node, message)) {
        return;
      }
      delete node.dataset.lazyRequestId;
