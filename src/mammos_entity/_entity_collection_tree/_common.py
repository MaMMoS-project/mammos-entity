...
class _CollapsibleTextSpec:
    ...
    preview_text: str
    expanded_text: str
    preview_is_truncated: bool = False
...
def _format_repr_summary(
    items: collections.abc.Sequence[object],
    opener: str,
    closer: str,
    format_item: collections.abc.Callable[[object], str] | None = None,
) -> str:
...
def _format_sequence_repr_summary(value: list | tuple) -> str:
    ...
    return _format_repr_summary(value, opener, closer)
...
def _format_dict_repr_summary(value: dict[object, object]) -> str:
    ...
    if len(value) <= 2 * _SUMMARY_EDGE_ITEMS:
        items = list(value.items())
    else:
        head = list(itertools.islice(value.items(), _SUMMARY_EDGE_ITEMS))
        tail = list(itertools.islice(reversed(value.items()), _SUMMARY_EDGE_ITEMS))
        items = [*head, _REPR_ELLIPSIS, *reversed(tail)]

    elements = ["..." if item is _REPR_ELLIPSIS else format_item(item) for item in items]
...
def _format_dict_repr_expanded(value: dict[object, object]) -> str:
    ...
    edge_items = _EXPANDED_THRESHOLD // 2
    if len(value) <= _EXPANDED_THRESHOLD:
        truncated_items = list(value.items())
    else:
        head = list(itertools.islice(value.items(), edge_items))
        tail = list(itertools.islice(reversed(value.items()), edge_items))
        truncated_items = [*head, (_REPR_ELLIPSIS, _REPR_ELLIPSIS), *reversed(tail)]
...
def _compact_value_text_spec(
    ...
    *,
    ...
    """Return a compact value spec, or ``None`` when formatting fails."""
    try:
        ...
        if isinstance(value, u.Quantity) and shape not in [(), None]:
            return _CollapsibleTextSpec(
                ...
                expanded_text=_format_quantity_repr_expanded(value) if include_expanded else "",
                preview_is_truncated=np.size(value.value) > 2 * _SUMMARY_EDGE_ITEMS,
        ...
        if isinstance(value, np.ndarray):
            return _CollapsibleTextSpec(
                ...
                expanded_text=_format_array_repr_expanded(value) if include_expanded else "",
                preview_is_truncated=value.size > 2 * _SUMMARY_EDGE_ITEMS,
                meta_text=f"shape={value.shape}",
            )
        if isinstance(value, list | tuple):
            return _CollapsibleTextSpec(
                preview_text=_format_sequence_repr_summary(value),
                expanded_text=_format_sequence_repr_expanded(value) if include_expanded else "",
                preview_is_truncated=len(value) > 2 * _SUMMARY_EDGE_ITEMS,
                meta_text=f"len={len(value)}",
            )
        if isinstance(value, dict):
            return _CollapsibleTextSpec(
                preview_text=_format_dict_repr_summary(value),
                expanded_text=_format_dict_repr_expanded(value) if include_expanded else "",
                preview_is_truncated=len(value) > 2 * _SUMMARY_EDGE_ITEMS,
                meta_text=f"len={len(value)}",
            )
    except Exception:
