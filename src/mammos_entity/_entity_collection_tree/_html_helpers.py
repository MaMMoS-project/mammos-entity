...
def _render_leaf_details(
    ...
    *,
    ...
    if node_id is not None:
        lazy_attrs = (
            f" data-lazy-id='{node_id}' data-lazy-patch='replace-self' data-lazy-loading='false' data-lazy-done='false'"
        )
    body_html = ""
    if include_body:
        body_html = (
...
def _render_widget_collection_node(summary_html: str, node_id: str, description: str, *, root: bool) -> str:
    ...
    lazy_attrs = (
        ...
        "data-lazy-loading='false' data-lazy-loaded='false' data-lazy-done='false' "
...
def _render_collection_controls(node_id: str, *, next_index: int, total: int, page_size: int) -> str:
...
def _entity_render_state(value: object, *, include_expanded: bool = True) -> _EntityRenderState:
    ...
    preview_is_truncated = has_shape and getattr(entity_value, "size", 0) > 2 * _SUMMARY_EDGE_ITEMS
    if include_expanded or not preview_is_truncated:
        ...
        value_text = ""
        compact_value_text = _format_array_repr_summary(entity_value)
        show_details = True
...
def _entity_requires_lazy_details(value: object) -> bool:
    ...
    return _entity_uses_details(_entity_render_state(value, include_expanded=False))


def _entity_collapsible_spec(state: _EntityRenderState, *, include_expanded: bool = True) -> _CollapsibleTextSpec:
    ...
    if state.unit_text:
        return _CollapsibleTextSpec(
            ...
            expanded_text=_format_array_repr_expanded(state.entity_value) if include_expanded else "",
            summary_unit_text=state.unit_text,
        )
    return _CollapsibleTextSpec(
        preview_text=_truncate_preview_text(state.compact_value_text),
        expanded_text=state.value_text if include_expanded else "",
    )
...
def _render_entity_details_html(
    ...
    return _render_leaf_details(
        _entity_collapsible_spec(state, include_expanded=include_body),
...
def _render_lazy_entity_value_html(value: object, entity_id: str) -> str:
    ...
    state = _entity_render_state(value, include_expanded=False)
...
def _leaf_render_state(value: object, *, include_expanded: bool = True) -> _LeafRenderState:
    """Collect shared rendering state for a non-entity leaf value."""
    compact_spec = None
    if isinstance(value, u.Quantity | np.ndarray | list | tuple | dict):
        compact_spec = _compact_value_text_spec(value, include_expanded=include_expanded)
        if not include_expanded and compact_spec is not None and compact_spec.preview_is_truncated:
    ...
    use_compact = compact_spec is not None and (repr_failed or len(repr_value_text) > _MAX_INLINE_CHARS)
    return _LeafRenderState(
...
def _render_expanded_leaf_value_html(value: object) -> str:
    ...
    if compact_spec is None:
        fallback_text, _ = _repr_with_fallback(value)
        compact_spec = _CollapsibleTextSpec(
            preview_text=_truncate_preview_text(fallback_text),
            expanded_text=fallback_text,
        )
    return _render_leaf_details(compact_spec, css_classes=_COMPACT_DETAILS_CSS_CLASSES, open_details=True)
...
def _render_leaf_value_html(value: object) -> str:
    ...
    if state.compact_spec is not None and state.use_compact:
